...
export default function LoginPage() {
  ...
  const validatePassword = (val: string) => {
    if (!val && authMode !== 'magic') return 'Password is required';
    if (val.length < 6 && authMode !== 'magic') return 'Password must be at least 6 characters';
    return null;
  };
  ...
  const handleStep1Submit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (userRoleTab === 'admin') {
        ...
        setIsLoading(false);
        // Move to Step 2: Consumer App Authentication for Company Founder
  ...
  const handleMagicLink = (e: React.FormEvent) => {
    e.preventDefault();
    const errEmail = validateEmail(email || 'alex@founderos.io');
    if (errEmail) {
      setEmailError(errEmail);
      return;
    }
    ...
    setTimeout(() => {
      setIsLoading(false);
      setToastMessage({ title: 'Passwordless magic link sent to your email!', type: 'success' });
    }, 800);
  };
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-r from-blue-50 via-slate-50 to-indigo-50 border border-blue-200 text-center space-y-2 shadow-sm">
                  ...
                  >
                    {isLoading ? (
                      ...
                      <>
                        <span>{userRoleTab === 'admin' ? 'Launch SaaS Creator Control Center' : 'Launch Executive Founder Workspace'}</span>
                  ...
                  <form onSubmit={handleStep1Submit} className="space-y-4 text-xs">
                    ...
                    >
                      {isLoading ? (
                        ...
                        <span>{authMode === 'signup' ? 'Create Founder Workspace →' : 'Sign In to Workspace →'}</span>
                      )}
                    </button>
                  </form>
                )}
